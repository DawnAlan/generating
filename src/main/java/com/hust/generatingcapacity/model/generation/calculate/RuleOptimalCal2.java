...
import com.hust.generatingcapacity.model.generation.dispatch.PiecewiseLambdas;
...
//        double eps = 0.05; // 计算最小发电量时，弃水约束应增大权重
        ...
        if (calParam.isConsiderH() && !calParam.isGenMin()) {//有细致的耗水率曲线且是计算最大发电
            ...
            PiecewiseLambdas lam = PiecewiseLambdas.addPiecewiseHa(model, Ha, Hgrid); // 构建 Ha 分段线性化骨架
            double[] Lgrid = waterConsumptionLine.stream().mapToDouble(cv -> cv.getValue() * 1e3).toArray();
            PiecewiseLambdas.linkLinearByLambda(model, Lc, lam, Lgrid); // 把 Lc 用同一套 λ 绑定
            double[] Vgrid = reservoirStorageLine.stream().mapToDouble(cv -> cv.getValue() * 1e6).toArray();
            PiecewiseLambdas.linkLinearByLambda(model, Va, lam, Vgrid); // 把 Va 用同一套 λ 绑定
            // McCormick：Vp = Gen * L耗
